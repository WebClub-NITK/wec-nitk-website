...
import { getSIGByBlogId } from '@/helpers/getSIGs';
...
export async function generateMetadata({ params }) {
  ...
  const cover_image = getStrapiMedia(
...
export default async function Page({ params }) {
  ...
  // Fetch SIG data for this blog to check if it has a calendar
  let sig = null;
  try {
    // Use blog.id since the filtering is by blog ID in the relationship
    sig = await getSIGByBlogId(blog.id);
  } catch (error) {
    console.error('Error fetching SIG data for blog:', error);
  }

  // fetch cover_image url
  let cover_image = getStrapiMedia(
    blog.attributes.cover_image.data?.attributes.url,
  );
  ...
  categories = categories.map((category) => {
    return (
      <Button
        key={category.id}
        className="text-xs py-1 px-2 rounded-3xl bg-primary-200/20"
        variant="ghost"
      >
  ...
  return (
    <>
      <section className="w-full lg:h-24 h-20 flex items-center justify-center bg-accent-900 bg-cover bg-center"></section>
      {/* Blog content */}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-2">
                ...
                <div className="flex gap-x-2 flex-row flex-wrap">
                  {written_by.data.map((writer, index) => {
                    return (
                      <span key={writer.id}>
                        <a
                          href={writer.attributes.linkedin_id}
                          target="_blank"
              ...
              {/* Google Calendar Section - Only show if SIG is found and has calendar ID */}
              {sig && sig.attributes?.google_calendar_id && (
                <GoogleCalendar
                  calendarId={sig.attributes.google_calendar_id}
                  title={`${sig.attributes.name} Events Calendar`}
                />
              )}
            </div>
